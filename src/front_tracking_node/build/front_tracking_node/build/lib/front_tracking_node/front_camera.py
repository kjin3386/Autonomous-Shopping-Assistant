...
# OC_SORT 모듈 경로 설정
ocsort_path = "/home/incsl-wego22/ws_capstone/src/front_tracking_node/3rdparty/OC_SORT"
    ...
    def process_frame(self):
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                detections_yolo.append([xyxy[0], xyxy[1], xyxy[2], xyxy[3], conf_score, cls_id])
        detections_np = np.array(detections_yolo)


        tracks = self.tracker.update(detections_np, img=frame)
        print(f"[OCSort DEBUG] tracks: {tracks}")
        person_array_msg = PersonTrackArray()

        for track in tracks:
            print("for debuuuuuuug")
            ...
            x = r_meters*math.cos(radians(theta_deg))
